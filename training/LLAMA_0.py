...
"""
# 3. 指令解析器
from jsonformer import Jsonformer  # 需安裝: pip install jsonformer
...
class CommandParser:
    ...
    def parse(self, command):
        ...
        #             <<任務>>
        #             從指令中提取：
        #             1. 物體：{'金元和' if '金元和' in command else 'Wafer' if 'Wafer' in command else '晶圓盒'}
        #             2. 站點：按出現順序提取

        #             <<輸出格式>>
        #             {{"object":"[物體]","start":"[第一站]","end":"[第二站]"}}

        #             <<示例>>
        #             指令：把第2站的晶圓盒搬到第三站
        #             輸出：start="第2站", end="第三站"
        #             """
...
if __name__ == "__main__":
    ...
    # 訓練模型
    trained_model = train_model(model, dataloader, epochs= 20)
